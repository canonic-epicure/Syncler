Class('Syncler.Replica', {
    ...
    methods : {
        ...
        write : function (spec) {
            ...
            // this will allow Mutation.Group to force the `checkPrecondition` call
            // which actually implement the `apply` phase in it 
            var forceCheckPreconditions = mutation.savePrecondition(this) === false
            mutation.apply(this, !forceCheckPreconditions)
        ...
        getLatestOwnMutation : function () {
            var tentativeQueue = this.tentativeQueue
            
            if (tentativeQueue.length) return tentativeQueue[ tentativeQueue.length - 1 ]
            
            var ownMutation 
            
            Joose.A.eachR(this.committedQueue, function (mutation) {
                if (mutation.clientID == this.clientID) {
        ...
        canUndo : function () {
            var latestOwnMutation = this.getLatestOwnMutation()
            
            return latestOwnMutation && latestOwnMutation instanceof Syncler.Mutation.UndoPoint
        },
        
        
        addUndoPoint : function (spec) {
            spec = spec || {}
            
            spec.type = 'Syncler.Mutation.UndoPoint'
