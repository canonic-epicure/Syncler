Class('Syncler.Test.Actor', {
    ...
    methods : {
        ...
        getRandomValue : function (replica, instance) {
            var type        = this.getRandomArrayValue([ 'object', 'array', 'number', 'attribute' ])
            
            if (type == 'object')       return new Syncler.Attribute.Object(replica, { key : Math.floor(Math.random() * 1e8) })
            if (type == 'array')        return new Syncler.Attribute.Array(replica, [ Math.floor(Math.random() * 1e8) ])
            
            if (type == 'number')       return Math.floor(Math.random() * 1e8)
            
            if (type == 'attribute')    return instance[
                this.getRandomArrayValue([
                    'attr1',
                    'attr2',
                    'attr3',
                    'attr4',
        ...
        getRandomObjectMutation : function (replica, instance, synclerObject) {
            ...
            if (objCount) {
                if (mutation == 'set') synclerObject.set(this.getRandomArrayValue(keys), this.getRandomValue(replica, instance))
                
                if (mutation == 'remove') synclerObject.remove(this.getRandomArrayValue(keys))
            } else {
